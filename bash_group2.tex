...
Here's a simple script to back up important files to FarmShare (this is just an example, note that GitHub, which you'll learn about later, provides a better way of backing up your work in many cases). 
...
today=$(date +%F)
\end{lstlisting}
This creates a variable called today, and stores the date in it. the \lstinline|$(...)| essentially tells bash to run the commands inside the parentheses and then stick the output into the rest of the line (i.e. save the output to the today variable here). 
\item
\begin{lstlisting}
tobackup=~/to_backup
server=your-SUNet-ID@corn.stanford.edu
...
\begin{itemize}
    \item \lstinline|tar czf - $tobackup| compresses the \lstinline|$tobackup| directory and passes the result to the stdout stream
    \item \lstinline!|! the \lstinline!|! is called a pipe. It takes the stdout stream and redirects it as input to the following command. 
    \item \lstinline|ssh $server ...| logs in to \lstinline|$server| using ssh, and then runs the commands that follow
    \item \lstinline|'( cd ~/Documents/backup_files/ ; cat > '${today}'.tar.gz )'| The quotes around everything except \lstinline|${today}| tell your computer to not run these commands, instead give them to ssh to run on the server. The \lstinline|cat > '${today}'.tar.gz| part tells the server to take the input it's given (from the pipe above) and save it out to the named file.
\end{itemize}
\end{itemize}
\end{frame}
